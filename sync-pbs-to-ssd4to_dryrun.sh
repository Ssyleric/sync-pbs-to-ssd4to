...
# rsync (DRY-RUN)
RSYNC_BASE_OPTS="-aHAX --numeric-ids --info=progress2 --stats --dry-run --itemize-changes -v"
RSYNC_SSH_OPTS="-i ${SSH_KEY} -o BatchMode=yes -o StrictHostKeyChecking=accept-new"

# log
LOG_DIR="/var/log"
LOG="${LOG_DIR}/sync-pbs-ssd4to_copy_dryrun_$(date +%F_%H-%M-%S).log"
...
mkdir -p "$LOG_DIR" "$DST"
umask 022
log "▶️ DRY-RUN COPIE PBS → PVE (aucune donnée copiée)"
...
### ======= DRY-RUN =======
log "🟦 DRY-RUN (rsync --dry-run, aucun write)…"
T_START=$(date +%s)
...
SUMMARY=$(
  cat <<EOF
🔍 DRY-RUN PBS → PVE (aucune donnée copiée)
• Source : ${PBS_IP}:${SRC}
• Destination : ${DST}
• Taille source : ${SRC_HUM}
• Taille destination actuelle : ${DST_HUM}
• Durée : ${DUR}s
• Log : ${LOG}
